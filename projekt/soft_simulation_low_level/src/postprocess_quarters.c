...
void postprocess_quarters(fixed_t* cos_reg, fixed_t* sin_reg, float* cos_res, float* sin_res, int8_t flips){
    ...
    switch(flips){
        ...
        case -2:
            ...
            *cos_res = (*cos_reg != (fixed_t) 0x80000000) ? -1 * (float) *cos_reg / (1 << WIDTH-1) : (float) *cos_reg / (1 << WIDTH-1);
            *sin_res = -1 * (float) *sin_reg / (1 << WIDTH-1);
            break;

        default:
            // Edge case for 0
            *cos_res = ( *cos_reg < 0 ) ? ( (float) -*cos_reg / (1 << WIDTH-1)) : ( (float) *cos_reg / (1 << WIDTH-1));
